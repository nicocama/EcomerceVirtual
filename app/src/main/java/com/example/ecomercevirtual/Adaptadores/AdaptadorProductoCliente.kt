...
import com.example.ecomercevirtual.Filtro.FiltroProducto
import com.example.ecomercevirtual.Modelo.ModeloProducto
import com.example.ecomercevirtual.R
import com.example.ecomercevirtual.databinding.ItemProductoClienteBinding
...
class AdaptadorProductoCliente : RecyclerView.Adapter<AdaptadorProductoCliente.HolderProducto>,
    Filterable {
    ...
    private fun comprobarFavorito(modeloProducto: ModeloProducto, holder: AdaptadorProductoCliente.HolderProducto) {
        val ref = FirebaseDatabase.getInstance().getReference("Usuarios")
    ...
    private fun visualizarDescuento(modeloProducto: ModeloProducto, holder: HolderProducto) {
        if (!modeloProducto.precioDesc.equals("0") && !modeloProducto.notaDesc.equals("")){
            ...
            holder.item_precio_p.paintFlags = holder.item_precio_p.paintFlags or Paint.STRIKE_THRU_TEXT_FLAG //Tachamos el precio
        }else{
            ...
            //Seteamos la informacion
            holder.item_precio_p.text = "${modeloProducto.precio}${" COP"}"
            holder.item_precio_p.paintFlags = holder.item_precio_p.paintFlags and Paint.STRIKE_THRU_TEXT_FLAG.inv() //Quitamos el tachado
        }
    }

    private fun cargarPrimeraImagen(modeloProducto: ModeloProducto, holder: AdaptadorProductoCliente.HolderProducto) {
